...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='Pandar40P_BluffBody_description').find('Pandar40P_BluffBody_description')
    default_model_path = os.path.join(pkg_share, 'urdf/Pandar40P_BluffBody.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/Pandar40P_BluffBody/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'Pandar40P_BluffBody', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
